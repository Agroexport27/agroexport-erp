...
export default function EmbarquesPage() {
  ...
  async function cargarRecientes() {
    ...
    const { data, error } = await supabase
      .from("remision_envio")
      .select(
        "id, fecha_empaque, manifiesto, caja_transporte, empaque, campos(nombre), cuadros(nombre), distribuidores(nombre), remision_detalle(cantidad_cajas, cantidad_bins), remision_envio_cuadro(cuadros(nombre))"
      )
  ...
  async function guardar() {
    ...
    if (!errRem && remision && cuadroIds.length > 0) {
      await supabase
        .from("remision_envio_cuadro")
        .insert(cuadroIds.map((id) => ({ remision_id: remision.id, cuadro_id: id })));
    }
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-campo-900">Captura de remisión</h1>
      ...
      <div className="card mb-4 grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 md:grid-cols-4">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-campo-600">Work Order / Lote (cuadro)</label>
          <MultiSelectCuadros opciones={cuadros} seleccionados={cuadroIds} onChange={setCuadroIds} />
        </div>
        <div>
